...
from waspy.iba.pellicle_entities import PellicleDriverUrls, PellicleData, Detector, PositionCoordinates
...
class PellicleSetup:
    ...
    def move(self, position: PositionCoordinates):
        if position is None:
            return
        logging.info("[WASPY.IBA.PELLICLE_SETUP] Moving pellicle system to '" + str(position) + "'")
    ...
    @preemptive
    def count(self):
        logging.info("[WASPY.IBA.PELLICLE_SETUP] acquiring till target")
